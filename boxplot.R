...
#Date: 12/06/2024
#Purpose: Boxplots of the expression profiles of gene of interest in normal versus affected patient samples
...
# Load the data 
df <- read.csv("expression.txt", sep = '\t', row.names = 1)

# Define sample groups
tumor_groups = c(rep("Affected_patients", 30), rep("Adj_normal", 162))

# Transpose the dataframe to get samples as rows
df_t <- as.data.frame(t(df))
df_t$Sample <- rownames(df_t)
df_t$TumorType <- tumor_groups
df_t$TumorType <- factor(df_t$TumorType, levels = c("Affected_patients", "Adj_normal"))
